---
title: "Bulk RNA-seq analysis of phosphatase-related genes"
...
volcano_pp2a_pancreatic <- ggplot(volcano_pp2a_data, aes(x = log2FoldChange, y = neg_log10_padj, color = direction)) +
  ...
  theme_publication() +
  labs(title = "Phosphatase-related genes - PAAD", x = "Log2FC (Tumor vs Normal)", y = "-Log10(padj)")
...
if (nrow(significant_genes) > 0) {
  ...
  violin_plot <- violin_data %>%
    ...
    labs(
      title = "Significantly changed phosphatase-related genes",
      x = "Condition",
...
violin_plot
  ...
  cat("No significantly changed phosphatase-related genes found.\n")
}
...
volcano_pp2a_colorectal <- ggplot(volcano_pp2a_data, aes(x = log2FoldChange, y = neg_log10_padj, color = direction)) +
  ...
  geom_text_repel(aes(label = gene_symbol), size = 3.5, max.overlaps = 20) +
  theme_publication() +
  labs(title = "Phosphatase-related genes - COAD", x = "Log2FC (Tumor vs Normal)", y = "-Log10(padj)")
...
# Create violin plot
  violin_colorectal <- violin_data %>%
    ...
    labs(
      title = "Significantly changed phosphatase-related genes - COAD",
      x = "Condition",
      y = "Expression (log2(count + 1))"
    )
violin_colorectal
} else {
  cat("No significantly changed phosphatase-related genes found in COAD\n")
}
# store results for the csv
...
volcano_pp2a_lung <- ggplot(volcano_pp2a_data, aes(x = log2FoldChange, y = neg_log10_padj, color = direction)) +
  ...
  geom_text_repel(aes(label = gene_symbol), size = 3.5, max.overlaps = 20) +
  theme_publication() +
  labs(title = "Phosphatase-related genes - LUAD", x = "Log2FC (Tumor vs Normal)", y = "-Log10(padj)")
volcano_pp2a_lung
...
if (nrow(significant_genes) > 0) {
  ...
  violin_lung <- violin_data %>%
    ...
    labs(
      title = "Significantly changed phosphatase-related genes - LUAD",
      x = "Condition",
      y = "Expression (log2(count + 1))"
    )
  ...
  cat("No significantly changed phosphatase-related genes found in LUAD\n")
}
# store results for the csv
